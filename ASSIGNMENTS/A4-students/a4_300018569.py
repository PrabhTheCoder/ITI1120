...
def create_network(file_name):
    ...
    #turns friends into a list of lists of ints instead of space seperated ints.
    #Also creates duplicates to facilitate for the fact that the file contains
    #no duplicate connections (I.e if [1,3] exists it adds [3,1].
    ...
    for i in range(len(friends)):
        ...
        friends.append([connection, curr_id])
    if len(friends) >= 2:
        friends.sort() 
        prev_user = friends[0][0] - 1 #makes sure curr_num is initialized as a member not in the 2D list
        index = -1

        #Populates the actual network with the newly conveniently sorted friends pair data.
        for i in range(len(friends)): 
            ...
            if prev_user == curr_user:
                ...
                prev_user = friends[i][0]
                index += 1
                network.append((curr_user,[connection]))
...
def get_uid(network):
    ...
    while index == -1:
        ...
        try:
            ...
            index = binary_search(network, uid)
